...
class Mail extends CI_Controller{
	...
	public function index(){
		$data = array(
			'menu' => 1,
			'data' => base_url('admin/mail/data/inbox')
		);
	...
	public function sent(){
		$data = array(
			'menu' => 2,
			'data' => base_url('admin/mail/data/sent')
		);
		$this->load->view('home', $data);
	}
	...
	public function trash($id=null){
		if(is_null($id)){
			$data = array(
				'menu' => 3,
				'data' => base_url('admin/mail/data/trash')
			);
			$this->load->view('home', $data);
		}
		else{
	...
	public function compose($id=null){
		...
		if($this->form_validation->run() == FALSE){
			if(!empty(set_value('dest'))){
				$user = $this->m_data->get_user(set_value('dest'));
			}
			$data = array('user' => $user);
			$this->load->view('compose', $data);
		}
		else{
			$user = $this->m_data->get_user($this->input->post('dest'));
	...
	public function dest(){
		if($this->input->is_ajax_request()){
			...
			$data = $this->db
				->select('id_user as id, nama as text')
				->where('role', 'user')
				->limit($limit)
				->like('nama', $query)
				->get('user_mail')
	...
	public function content($id=null){
		...
		if($ref == base_url('admin/mail/view/'.$id)){
			...
			$columns = array(
				array(
					...
					'formatter' => function($d, $row){
						...
						return 
						'
						<a href="'.base_url('admin/mail/view/'.$d).'">
							<div class="w-100">
								<p class="m-0">
